...
func (b *Btelegram) maybeConvertTgs(name *string, data *[]byte) {
	format := "webp" //b.GetString("MediaConvertTgs")
...
func (b *Btelegram) handleContactMessage(msg *config.Message, chatid int64) (string, error) {
    ...
    out := false
    foundFN := false
    var re = regexp.MustCompile(`TEL|iteml.TEL`)
    ...
    for parts[i] != "FN" {
        ...
        if parts[i] == "FN"{
            foundFN = true
            n = i
            for !(re.MatchString(parts[n])) {
                b.Log.Debugf("partsN: %s",parts[n])
                n += 1
                if (n >= len(parts)) {
                  out = true
                  b.Log.Debugf("broke 1")
                  break
                 }
            }
          if (out == true){
          b.Log.Debugf("broke 2")
          break
          }
   ...
   // b.Log.Debugf("Contact message made: %+v",cfg)
    ...
    b.Log.Debugf("Sending contact message: %+v",cfg)
...
func (b *Btelegram) handleUploadFile(msg *config.Message, chatid int64, parentID int) (string, error) {
	...
	for _, f := range msg.Extra["file"] {
		...
		case ".gif":
		    gc := tgbotapi.NewAnimation(chatid, file)
			gc.Caption, gc.ParseMode = TGGetParseMode(b, msg.Username, fi.Comment)
			gc.ReplyToMessageID = parentID
			res, err := b.c.Send(gc)
			if err != nil {
				return "", err
			}
			return strconv.Itoa(res.MessageID), nil
			//gc := tgbotapi.NewInputMediaPhoto(file)
			//if fi.Comment != "" {
			//	gc.Caption, gc.ParseMode = TGGetParseMode(b, msg.Username, fi.Comment)
			//}
